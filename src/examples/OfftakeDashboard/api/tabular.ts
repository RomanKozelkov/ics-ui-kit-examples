...
export function getValueColumn(metric: Metric): TabularColumnRef {
	if (metric === "Units") {
		return { column: { table: "Offtake~Tabular", name: "Offtake, unit" } };
	}
	if (metric === "USD") {
		return { column: { table: "Offtake~Tabular", name: "Offtake, USD conversion" } };
	}
	return { column: { table: "Offtake~Tabular", name: "Offtake, INV RUB" } };
}
...
export function buildTableFilter(input: BuildFilterInput): TabularFilter {
	...
	if (input.contractValues && input.contractValues.length > 0) {
		main.push({
			op: "in",
			column: { table: "Client Contract~Tabular", name: "Client is Contracted" },
			list: input.contractValues
		});
	}
	if (input.salesChannelValues && input.salesChannelValues.length > 0) {
		main.push({
			op: "in",
			column: { table: "Client Ecom Typed Filter~Tabular", name: "Filter" },
			list: input.salesChannelValues
		});
	}
